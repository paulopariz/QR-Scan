<template>
  <div class="fixed bottom-8 w-screen">
    <div class="flex justify-center flex-col items-center gap-11 w-screen">
      ...
      <button
        @click="startClick"
        class="bg-dark flex items-center justify-between w-1/4 px-7 py-4 select-none rounded-lg"
      >
        <span class="text-transparent text-[0px]">.</span>
        <h1 class="text-white font-bold tracking-wide text-lg">{{ startText }}</h1>
        <img src="@/assets/img/arrow.svg" class="" alt="Arrow" />
      </button>
    </div>
  </div>
</template>
